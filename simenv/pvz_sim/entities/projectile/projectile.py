...
class Projectile(Entity):
    ...
    def __init__(self, speed, lane, pos):
        ...
        self._speed = speed
        self._pos = pos
        self._offset = 0.  # continuous offset for precise movement
    ...
    def _hit(self, zombie):
        """Check if projectile hit a zombie using lexicographic segment check."""
        if zombie.lane == self.lane:
            if ((self._previous_pos,
                 self._previous_offset * zombie.WALKING_SPEED * config.FPS)
                <= (zombie.pos, zombie._offset)
                and (self._pos,
                     self._offset * zombie.WALKING_SPEED * config.FPS)
                >= (zombie.pos, zombie._offset)):
                return True
        return False
